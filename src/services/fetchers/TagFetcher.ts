...
export default class TagFetcher {
  ...
  static updateOneByName(name: string, data: Partial<Pick<ITag, "name" | "description">>) {
  ...
  static async getTagsRelatedToProduct(productName: string, query: string = "") {
    return (await axiosWithAuth.get<{
      items: ITag[],
      count: number
    }>(`/products-have-tags/products/${productName}` + (query ? `?${query}` : ""))).data
  }
  ...
  static async create(data: Pick<ITag, "name" | "description">) {
